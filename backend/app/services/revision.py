...
def build_realtime_revision_prompt(domain: str, source_language: str, target_language: str) -> str:
    # 经真实 qwen-flash 验证：仅强调「保守」会让模型对明显错误也按兵不动；
    # 这里改为「不做风格润色」+「列出应当修正的错误类别与示例」，使明确错误可靠触发、
    # 干净译文零误纠（术语/数字单位错→纠正 conf≈0.98，问候/常规技术句→空）。
    return "\n".join(
        [
            "你是实时同声传译的「在线纠偏」模块，在传译进行中复核最近输出的译文。",
            f"领域：{domain}；源语言：{source_language}；目标语言：{target_language}。",
            f"领域关注点：{domain_focus(domain)}",
            "给你最近几句的「原文 + 当前译文」，最后一句是最新上下文。",
            "任务：结合最新上下文，找出**之前的句子**里必须修正的明确错误。",
            "应当修正（发现即改）：",
            "- 一词多义选错义项（如技术语境把 Transformer 误作「变压器」、token 误作「代币」、bank 误作「河岸」）；",
            "- 人名/地名/机构/产品/专有名词译错或前后不一致；",
            "- 数字、单位、日期、金额错误；否定/肯定或主客体关系译反；术语前后不一致。",
            "不要修正：仅措辞/风格/语序偏好（译文已准确就别动）；最后一句；没把握的猜测。",
            "修正时给出完整替换后的整句译文（afterText）与 confidence(0~1)。",
