...
每个 Issue 必须对应一个 PR；PR 描述和评论要持续记录实现过程。后端或算法改动必须附可复现实验：说明数据规模、基线算法、优化动作、运行命令、平均耗时/吞吐和结果，不能只写“性能提升”。
...
Issue 和 PR 统一使用一行标题：`<gitemoji> <type>(<scope>): <中文动词短语>`。`type` 使用 `feat`、`fix`、`refactor`、`docs`、`perf`、`test`、`ci`、`chore` 或 `security`；`scope` 使用小写英文模块名（如 `web`、`backend`、`pipeline`、`ws`、`history`），没有明确模块时可以省略。标题主体应以中文动词开头，避免乱码、无意义英文和句末标点。

同一变更的 Issue 与 PR 必须共享相同的 gitemoji、type、scope 和中文标题主体，只允许 PR 描述额外补充实现细节。示例：`🐛 fix(ws): 防止非法 media_clock 中断连接`。创建或编辑元数据后，应确认 GitHub 页面实际显示为 UTF-8 中文。
...
PR 评论用于持续记录实验，不要只在最后一次提交里补一段总结。至少覆盖架构决策、实验过程和验证收尾三个事实节点；如果出现新的 CI 结果、审查意见、失败实验或回滚决策，应继续追加评论。评论数量不设人为上限，但每条都必须对应真实改动或可复现证据，不能为了数量灌水。每次有意义的实验都用固定格式：
...
涉及性能、稳定性或缺陷修复时，追加 STAR 记录：

```text
Situation（情境）：<现象、规模、影响>
Task（任务）：<目标和验收指标>
Action（行动）：<数据结构、算法、设计模式或代码改动>
Result（结果）：<基线与优化后数据、测试、残余风险>
```
