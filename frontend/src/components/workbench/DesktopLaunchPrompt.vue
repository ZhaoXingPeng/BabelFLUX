...
<template>
  <section class="desktop-launch-prompt" role="status" aria-live="polite">
    <div>
      ...
      <span v-else-if="state === 'launched'">已打开客户端窗口，可在悬浮条中选择音源。</span>
      <span v-else>正在等待系统协议响应。</span>
    </div>
    <div class="desktop-launch-actions">
      <button v-if="state !== 'launched'" class="stage-button" type="button" @click="emit('retry')">重试唤起</button>
      <button v-if="state === 'fallback'" class="stage-button" type="button" @click="emit('reopen')">
        我已安装，直接打开
      </button>
      <button v-if="state !== 'launched'" class="stage-button primary" type="button" @click="emit('continueWeb')">
        继续网页悬浮
      </button>
      <a v-if="state !== 'launched' && deepLinkUrl" class="stage-button" :href="deepLinkUrl">打开协议</a>
